...
import { ChevronLeft, ChevronRight, LayoutGrid, List, SortAsc, Filter, Lock, Sparkles, Star, Users, Briefcase, MapPin, X, Loader2 } from 'lucide-react';
import { JobCard } from '../../../components/ui/JobCard';
import JobDetailDrawer from './JobDetailDrawer';
import JobListingLoader from '../../../components/ui/JobListingLoader';
...
interface JobListingsProps {
  ...
  onSourceFilterChange: (filter: 'internal' | 'external') => void;
  onLoadMore: () => void;
  hasMoreJobs: boolean;
}
...
const JobListings: React.FC<JobListingsProps> = ({
  ...
  // Infinite scroll detection for external jobs
  useEffect(() => {
    if (sourceFilter !== 'external' || !hasMoreJobs || loadingMore) return;
  ...
  }, [sourceFilter, hasMoreJobs, loadingMore, onLoadMore]);
      ...
      <div className="flex-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
          <div className="flex items-center gap-6">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-lg">
                {isLoggedIn ? (
                  sourceFilter === 'external' ? (
                    <>
                      Showing <span className="font-semibold text-teal-600 dark:text-cyan-400">{jobs.length.toLocaleString()}</span> of <span className="font-semibold text-teal-600 dark:text-cyan-400">{totalJobs.toLocaleString()}</span> external jobs
                    </>
                  ) : (
                    <>
            ...
            {/* Show/Hide Filters Button removed as per request */}
          </div>
        ...
        {/* Source Filter Tabs */}
        <div className="flex items-center gap-2 mb-6 p-1 bg-gray-100 dark:bg-slate-800 rounded-xl w-fit">
          <button
        ...
        <div className="relative">
          {/* Show loader when loading, otherwise show job cards */}
          {loading ? (
            <JobListingLoader count={jobsPerPage} />
          ) : (
            <div className={`mb-12 ${
              viewMode === 'grid' 
          ...
          {!isLoggedIn && hiddenJobsCount > 0 && (
            <div className="relative">
              ...
              <div className={`relative ${
                ...
                {Array.from({ length: Math.min(viewMode === 'grid' ? 6 : 10, hiddenJobsCount) }).map((_, index) => (
                  <div
                    key={`preview-${index}`}
        ...
        {/* Load More for External Jobs / Pagination for Internal Jobs */}
        {sourceFilter === 'external' ? (
          /* Infinite Scroll for External Jobs */
          <div className="flex flex-col items-center mt-12 space-y-4">
            ...
            {hasMoreJobs && !loadingMore && (
              <button
                onClick={onLoadMore}
                className="px-8 py-4 bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white font-semibold rounded-xl transition-all transform hover:scale-105 shadow-lg hover:shadow-emerald-500/25"
              >
                Load More Jobs
              </button>
            )}
            ...
            {!hasMoreJobs && jobs.length > 0 && (
              <div className="text-center py-8">
                <div className="inline-flex items-center px-6 py-3 bg-gray-100 dark:bg-slate-800 rounded-xl">
          ...
          /* Traditional Pagination for Internal/Mixed Jobs */
                  ...
                  <button
                    ...
                    className={`px-5 py-3 rounded-xl font-semibold text-sm transition-all ${
                      currentPage === page
                        ...
                        : 'bg-white dark:bg-slate-800 text-gray-700 dark:text-gray-300 border-2 border-gray-200 dark:border-slate-700 hover:border-emerald-300 dark:hover:border-emerald-500 hover:text-emerald-700 dark:hover:text-emerald-400'
                    }`}
                  >
                    {page}
                  </button>
